...
static RunningJob *find_job_slot(int64_t job_id) {
    for (int i = 0; i < MAX_RUNNING_JOBS; i++)
        if (g_jobs[i].active && g_jobs[i].job_id == job_id) return &g_jobs[i];
    return NULL;
}
...
static void *runner_thread(void *arg) {
    ...
    /* Fetch job from DB */
    Job j;
    if (db_job_get(job_id, &j) != 0) return NULL;
    ...
    /* Write the command line to log */
    if (log_fd >= 0) {
        dprintf(log_fd, "CMD:");
        for (int i = 0; i < argc; i++) dprintf(log_fd, " %s", argv[i]);
        dprintf(log_fd, "\n\n");
    }

    /* fork + exec */
    int pipefd[2];
    if (pipe(pipefd) != 0) { db_job_set_finished(job_id, "failed", -1); return NULL; }
    pid_t pid = fork();
    if (pid == 0) {
        /* child */
        close(pipefd[0]);
    ...
    if (slot) { slot->job_id = job_id; slot->pid = pid; slot->active = 1; }
    pthread_mutex_unlock(&g_lock);

    db_job_set_started(job_id);

    /* Relay subprocess stdout → log file */
    char buf[1024];
    ssize_t n;
    while ((n = read(pipefd[0], buf, sizeof(buf))) > 0)
        if (log_fd >= 0) { if (write(log_fd, buf, (size_t)n) != n) {} }
    close(pipefd[0]);
    ...
    /* Notify SSE connections that stream has ended */
    pthread_mutex_lock(&g_lock);
    for (int i = 0; i < MAX_SSE_CONNS; i++) {
        if (g_sse[i].active && g_sse[i].job_id == job_id) {
            /* Mark for closure; job_sse_poll will send [DONE] */
            g_sse[i].active = 2; /* 2 = flush-and-close */
        }
    }
    pthread_mutex_unlock(&g_lock);
...
int job_run(int64_t job_id, const char *binary,
    ...
    pthread_mutex_lock(&g_lock);
    if (find_job_slot(job_id)) { pthread_mutex_unlock(&g_lock); return -1; }
    pthread_mutex_unlock(&g_lock);
    ...
    if (pthread_create(&tid, &attr, runner_thread, ta) != 0) {
        free(ta); pthread_attr_destroy(&attr); return -1;
    }
...
/* Called from the Mongoose timer (single-threaded Mongoose context) */
void job_sse_poll(void) {
    ...
    for (int i = 0; i < MAX_SSE_CONNS; i++) {
        ...
        char log_path[512];
        /* We need log_dir — store it globally */
        ...
        if (f) {
            ...
            while (fgets(line, sizeof(line), f)) {
                /* Strip trailing newline for SSE */
        ...
        if (sc->active == 2) {
            /* Job finished — send end marker and close */
